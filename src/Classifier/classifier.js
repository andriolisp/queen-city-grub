...
Classifier.classify = function (message) {
  ...
  recommendValue === locationValue && foodValue === foodTypeValue) {
    isRecommend = true
  }
  var result = {
    'message': message,
    'highend': Classifier.getValue(message, adjectives) != null || intentValue === 'highend',
    'recommend': isRecommend,
    'food': Classifier.getValue(message, food),
    'location': Classifier.getValue(message, neighborhoods)
  }
  return result
}

Classifier.getValue = function (message, data) {
  var val = null
...
console.log(Classifier.classify('pizza'))
